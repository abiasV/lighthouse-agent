import test from "node:test";
import assert from "node:assert/strict";

import createInitialState from "../src/state/createInitialState.js";

import runAgent from "../src/agent/runAgent.js";

test("real demand API failure becomes N_A and agent continues safely", async () => {
  const failingResearchClient = {
    responses: {
      create: async () => {
        throw new Error("API unavailable");
      },
    },
  };

  let state = createInitialState("Birthday Invitation");

  state.mockScenarios.competition = "GOOD_DATA";

  state = await runAgent(state, {
    useRealDemand: true,

    demandResearchClient: failingResearchClient,

    sleepFn: async () => {},
  });

  assert.equal(state.criteria.demand.status, "N_A");

  assert.equal(state.criteria.demand.confidence, "LOW");

  assert.equal(state.criteria.demand.score, null);

  assert.equal(
    state.criteria.demand.failureReason,
    "REAL_DEMAND_RESEARCH_FAILED",
  );

  assert.equal(state.pendingInput?.field, "currency");
});

test("invalid signal JSON becomes N_A and agent continues safely", async () => {
  const fakeResearchClient = {
    responses: {
      create: async () => {
        return {
          output_text: `
Search interest is substantial.
Commercial intent is visible.
`,
          output: [
            {
              type: "web_search_call",
              action: {
                sources: [
                  {
                    type: "url",
                    url: "https://www.etsy.com/example",
                  },
                  {
                    type: "url",
                    url: "https://trends.google.com/example",
                  },
                  {
                    type: "url",
                    url: "https://www.apple.com/example",
                  },
                ],
              },
            },
          ],
        };
      },
    },
  };

  const badSignalClient = {
    responses: {
      create: async () => {
        return {
          output_text: "this is not valid json",
        };
      },
    },
  };

  let state = createInitialState("Birthday Invitation");

  state.mockScenarios.competition = "GOOD_DATA";

  state = await runAgent(state, {
    useRealDemand: true,

    demandResearchClient: fakeResearchClient,

    demandSignalClient: badSignalClient,

    sleepFn: async () => {},
  });

  assert.equal(state.criteria.demand.status, "N_A");

  assert.equal(state.criteria.demand.confidence, "LOW");

  assert.equal(state.criteria.demand.score, null);

  assert.equal(
    state.criteria.demand.failureReason,
    "REAL_DEMAND_RESEARCH_FAILED",
  );

  assert.equal(state.pendingInput?.field, "currency");
});
