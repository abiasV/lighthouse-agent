...
const sections = {
  terms: [
    ...
    ["What you receive", "Lighthouse can import your shop name and active listing titles through a read-only Etsy connection. When sales import is enabled, Lighthouse reads shop receipts to calculate paid units for your chosen UTC dates and the preceding period. Cancelled and fully refunded orders are excluded; products affected by partial refunds require manual review. You supply date-range views, missing sales figures, product facts and a specific problem. When the private AI review is available, you can request draft copy, a suggested action and a measurement plan. Lighthouse does not publish edits, place orders, purchase advertising or hire people on your behalf."],
  ...
  privacy: [
    ...
    ["Information processed", "This includes Etsy account identifiers and profile information returned during verification, access and refresh tokens, shop names and active listing information returned during import. We also process the product facts, reporting dates, aggregate views and sales, problem descriptions and follow-up notes you enter. Sales import reads Etsy receipts temporarily to calculate per-product totals. Receipt responses may contain buyer identifiers, contact details, addresses and messages; these fields are discarded, not logged or stored by the sales importer, not returned to your browser and not sent to OpenAI. Only aggregate product figures enter the planner. Do not enter buyer information, passwords or payment-card details into free-text fields."],
